  ...
  return (
    <View style={styles.progressRow}>
      <View style={styles.progressLabelRow}>
        ...
        <Text style={[styles.progressValue, over && { color: theme.colors.accentWarn }]}>{Math.round(value)}{unit} / {Math.round(goal)}{unit}</Text>
      </View>
...
function generateAIReport({ consumed, burned, protein, carbs, fat, steps, proteinGoal, carbsGoal, fatGoal, calorieTarget }) {
  ...
  if (proteinPct < 0.7) insights.push({ icon: "🥩", text: `Protein intake is at ${Math.round(proteinPct * 100)}% of your goal. Try adding dal, paneer, curd, eggs, or chicken to meet your target.` });
...
export default function DailyReportScreen() {
  ...
  const distanceKm = log?.distanceKm ?? 0;
  const net = consumed - burned;
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          ...
          >
            ...
            {item.isToday && <View style={[styles.todayDot, selectedDate === item.key && { backgroundColor: "#fff" }]} />}
          </TouchableOpacity>
        )}
      />
        ...
        >
          <View style={styles.heroCard}>
            <View style={styles.heroRow}>
              ...
              <Text style={styles.heroMath}>−</Text>
              <View style={styles.heroItem}>
              ...
              <View style={styles.heroItem}>
                ...
                <Text style={[styles.heroValue, { color: net <= 0 ? theme.colors.primary : theme.colors.accentWarn }]}>{net > 0 ? "+" : ""}{net}</Text>
                <Text style={styles.heroLabel}>Net kcal</Text>
              </View>
            </View>
            <View style={styles.heroTrack}>
              <View style={[styles.heroFill, { width: `${Math.min((consumed / calorieTarget) * 100, 100)}%`, backgroundColor: consumed > calorieTarget ? theme.colors.accentWarn : theme.colors.primary }]} />
            </View>
            <Text style={styles.heroSub}>{consumed} of {calorieTarget} kcal daily target</Text>
          </View>
          ...
          <View style={styles.macroBarCard}>
            ...
            <ProgressBarRow label="Fat" value={fat} goal={fatGoal} color="#9C7A2F" unit="g" />
          ...
          <View style={styles.aiCard}>
            ...
            {aiInsights.map((ins, i) => (
              <View key={i} style={[styles.aiRow, i < aiInsights.length - 1 && styles.aiDivider]}>
                <Text style={styles.aiIcon}>{ins.icon}</Text>
                <Text style={styles.aiText}>{ins.text}</Text>
              </View>
            ))}
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  heroLabel: { fontSize: 10, color: theme.colors.textMuted },
  heroMath: { fontSize: 20, color: theme.colors.textMuted, paddingHorizontal: 4 },
  heroTrack: { height: 8, backgroundColor: theme.colors.border, borderRadius: 4 },
  heroFill: { height: "100%", borderRadius: 4 },
  heroSub: { fontSize: 11, color: theme.colors.textMuted, marginTop: 6, textAlign: "center" },
  ...
  aiDivider: { borderBottomWidth: 1, borderBottomColor: "#ffffff15" },
  aiIcon: { fontSize: 16 },
